...
BEGIN
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'server_admin') THEN
        CREATE ROLE server_admin LOGIN PASSWORD 'e21QQf34WW0';
    END IF;
END$$;
...
BEGIN
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'api_user') THEN
        CREATE ROLE api_user LOGIN PASSWORD 'fdhkD2gkfd848ff';
    END IF;
END$$;

-- 3. Назначить владельцем всех таблиц server_admin (повторить для каждой таблицы)
ALTER TABLE IF EXISTS users OWNER TO server_admin;
ALTER TABLE IF EXISTS audit_log OWNER TO server_admin;

-- 4. Дать server_admin все права на все таблицы
...
-- 5. Дать api_user только SELECT, INSERT, UPDATE на таблицу users
REVOKE ALL ON TABLE users FROM api_user;
GRANT SELECT, INSERT, UPDATE ON TABLE users TO api_user;

-- 6. Запретить api_user удалять, изменять структуру и выполнять опасные операции
REVOKE DELETE, TRUNCATE, REFERENCES, TRIGGER ON TABLE users FROM api_user;
REVOKE CREATE ON SCHEMA public FROM api_user;

-- 7. (Опционально) Ограничить подключение server_admin только с localhost (делается в pg_hba.conf на сервере)
